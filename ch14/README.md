...
- 외부 API를 연동하여 사용하는 방법을 알아봤다.
- 컴포넌트에서 API를 연동하여 개발할 때 유의 사항은 `useEffect에` 등록하는 함수는 `async`로 작성하면 안 된다는 점이다.
- 그 대신 함수 내부에 `async` 함수를 따로 만들어 줘야 한다.
- 추후에 사용해야 할 API가 많아지면 요청을 위한 상태 관리가 번거로워 지는데 이는 리덕스와 리덕스 미들웨어를 배우면 좀 더 쉽게 관리할 수 있다.
